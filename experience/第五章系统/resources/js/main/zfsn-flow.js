window.FLOW = {
	...
	createNewNode: function(nodeType, pos) {
		...
		// 1、创建前的检查
		var msg = graphUtil.checkGraphBeforeCreate(nodeType);  //只允许一个开始节点
		if (msg != '0') {
		...
		// 4、根据节点类型获取要渲染的节点对象
		var renderNode = FLOW.getRenderNodeFromType(nodeType);
	...
	_createLane: function(renderNode) {
		...
		// 添加到画板中
		var textArr = renderNode.text.split(''), i, tempText = '', a, b, c;
		...
		// 设置节点位置
		var t = renderNode.nodeType == 'broadwiseLane' ? renderNode.locTop : canvasY;
		var l = renderNode.nodeType == 'broadwiseLane' ? canvasX : renderNode.locLeft;
	...
	registerNodeEvent: function(tempId) {
		var selector = strUtil.isBlank(tempId) ? '.moveLight' : ZFSN.getJQSel(tempId);
		var $this = FLOW;
	...
	loadJson: function(data) {
		...
		// 节点数组
		var nodeArr = data.nodeDataArray;
		// 连线数组
		var linkArr = data.linkDataArray;
		// console.log(nodeArr[1].text);
		// 1、渲染节点到画布
		for (var i = 0, len = nodeArr.length; i < len; i++) {
			// 根据节点类型获取要渲染的节点对象
			var renderNode = FLOW.getRenderNodeFromType(nodeArr[i].nodeType);
	...
	reloadJson: function(data) {
		...
		for (var i = 0, len = nodeArr.length; i < len; i++) {
			...
			nodeArr[i].cla = renderNode.cla;
			nodeArr[i].icon = renderNode.icon;
			console.log("nodeArr"+nodeArr[i]);
			FLOW._createFlowNode(nodeArr[i],400);
			//FLOW._createNewNode(nodeArr[i]);
		}
		console.log("33333");
		console.log(nodeArr);
		...
		for (var i = 0, len = linkArr.length; i < len; i++) {
			...
			plumbUtil.connectFlowNode(linkArr[i].from, linkArr[i].to, linkArr[i].routerId, linkArr[i].sourceAnchors, linkArr[i].targetAnchors);
	...
	_createFlowNode: function(renderNode,base) {
		var _base = FLOW._base;

		// 2、添加节点到画布
		if(renderNode.nodeType=='start'){$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" >' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='end'){$("#collapseOne").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" >' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='positionModel'){
			$("#collapseOne").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" >' +
				'<span class="nodeTextCla">' + renderNode.text + '</span>' +
				renderNode.icon +
				'</div>'
			)
		...
		$("#"+renderNode.key).attr("param",renderNode.param);
		// 3、设置节点位置
		$(ZFSN.getJQSel(renderNode.key)).offset({ top: base + renderNode.locTop, left: renderNode.locLeft });
	...
	in_sute_createNewNode: function(renderNode,top,left,name) {
		var _base = FLOW._base;
		// 2、添加节点到画布
		// console.log(renderNode.cla);
		$("#in_sute_flow").append('<div id="' + renderNode.key + 'in_sute'+name+'" class="' + renderNode.cla + '" >' +
			'<span class="nodeTextCla">' + name + '</span>' +
			renderNode.icon +
			'</div>'
		);
		// $("#"+renderNode.key).attr("param",renderNode.param);
		// 3、设置节点位置
		$(ZFSN.getJQSel(renderNode.key+"in_sute"+name)).offset({ top: top, left: left });
		// console.log($(ZFSN.getJQSel(renderNode.key+"in_sute")).offset());
		// 4、设置节点的样式
		$(ZFSN.getJQSel(renderNode.key+"in_sute"+name)).css({
			'background': colorUtil.getNodeLinearBgFromHex(renderNode.bgColor),
			'height': renderNode.nodeHeight,
			'width': renderNode.nodeWidth,
			'line-height': renderNode.nodeHeight
		...
		// 5、设置节点的属性
		$(ZFSN.getJQSel(renderNode.key+"in_sute"+name)).attr('bgColor-gradient', renderNode.bgColor);

		// 10、将节点添加到图对象
		_base.graph.setNode(renderNode.key+"in_sute"+name, {
			text: renderNode.text,
			key: renderNode.key,
			nodeType: renderNode.nodeType,
			locTop: renderNode.locTop,
	...
	_createNewNode: function(renderNode) {
		...
		// 2、添加节点到画布
		if(renderNode.nodeType=='start')
		{$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setStartNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='end')
		{$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setEndNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		...
		{
			$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setPositionModelNodeAttr(\'' + renderNode.key + '\')">' +
				'<span class="nodeTextCla">' + renderNode.text + '</span>' +
				renderNode.icon +
				'</div>'
			)
		}
		else if(renderNode.nodeType=='positionQuery'){
			$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setPositionQueryNodeAttr(\'' + renderNode.key + '\')">' +
				'<span class="nodeTextCla">' + renderNode.text + '</span>' +
				renderNode.icon +
				'</div>'
			)
		}
		else if(renderNode.nodeType=='positionRatioQuery'){
			$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setPositionRatioNodeAttr(\'' + renderNode.key + '\')">' +
				'<span class="nodeTextCla">' + renderNode.text + '</span>' +
				renderNode.icon +
				'</div>'
			)
		}
		else if(renderNode.nodeType=='exceptionModel'){$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setExceptionModelNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='exceptionQuery'){$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setExceptionQueryNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='flightQuery'){$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setFlightQueryNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='simpleQuery'){$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setSimpleQueryNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='filter'){$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setFilterNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
		else if(renderNode.nodeType=='continue'){$("#Container").append('<div id="' + renderNode.key + '" class="' + renderNode.cla + '" onClick="attrCfgUtil.setContinueNodeAttr(\'' + renderNode.key + '\')">' +
			'<span class="nodeTextCla">' + renderNode.text + '</span>' +
			renderNode.icon +
			'</div>'
		)}
	...
	save: function() {
		// 1、检查流程图合法性
		var checkMsg = graphUtil.checkGraph();
		if (checkMsg != '0') {
	...
	/*	function downloadJson(data) {
			...
			var json = encodeURI(str,"utf-8");

		$.ajax({
			type: 'POST',
			url: '/in_sute_demo/saveJson.do',
	...
	in_sute_flow: function(){           //接收业务拓扑数据
		var obj = FLOW.getCurrentFlow();
		var nodeArr = obj.nodeDataArray;
		$("#in_sute_flow").empty();
		var left = 500;
		for( var i=0;i<nodeArr.length;i++){
			if(nodeArr[i].nodeType!="start"&&nodeArr[i].nodeType!="end"){ //找到业务算子
				console.log(nodeArr[i].nodeType);
				var renderNode = FLOW.getRenderNodeFromType(nodeArr[i].nodeType);
				nodeArr[i].cla = renderNode.cla;
				nodeArr[i].icon = renderNode.icon;
				if(nodeArr[i].nodeType=='positionModel'){
					// for(var a = 0;a<3;a++){
						for(var j=0;j<FLOW.in_sute_position.length;j++){
							console.log(j);
							FLOW.in_sute_createNewNode(nodeArr[i],800,left+j*200,FLOW.in_sute_position[j]);
						}
					// }

				}
				else if(nodeArr[i].nodeType=='positionQuery'){
					for(var j=0;j<FLOW.in_sute_positionQuery.length;j++){
						console.log(j);
						FLOW.in_sute_createNewNode(nodeArr[i],800,left+j*200,FLOW.in_sute_positionQuery[j]);
					}
				}
			}
		}
	...
	save2Photo: function() {
		// 1、检查当前流程图是否可以保存为图片
		var checkMsg = graphUtil.checkGraphBySave2Photo();
		if (checkMsg != '0') {
			layer.msg(checkMsg, { icon: 2 });
			return;
		}
		...
		// 2、计算生成图片的尺寸
		var positionObj = graphUtil.getCanvasSizeByNode();
		
		// 3、处理svg标签，这里只做对连接线的转换，端点暂不考虑
	    var removeArr = [];
	    var svgElem = $("#canvasId").find('svg[id]');
	    var i;
	    svgElem.each(function(index, node) {
	    	// 3.1、创建canvas标签，设置标签id并将id放入移除数组中，便于生成图片后移除canvas
	    	var canvas = document.createElement('canvas');
	    	var canvasId = 'canvas-' + ZFSN.getUUID();
	    	canvas.id = canvasId;
		...
		html2canvas(document.getElementById('Container'), {
			...
			var alinkId = 'alink-' + ZFSN.getUUID();
	...
	clearAllTimer: function() {
		var _base = FLOW._base;
		var allTimer = _base.allTimer;
		
		// 1、点击画布时清除所有的定时器
		for(var timerName in allTimer) {
	...
	/*newTask: function(){
		...
		if (saveStatus == 'block') {
			layer.confirm(CONFIG.msg.newConfirm, { icon: 7, title: '提示' }, function(index) {
				uploadFlow();
				FLOW.removeAll();
				layer.close(index);
			});
	...
	connectionTool: function() {
		...
		$.each(nodeArr, function(index) {
			...
			console.log($("#"+nodeArr[index]).offset());
			...
			console.log(_base.plumb.isSource("#"+nodeArr[index]));
			...
			console.log(_base.plumb.isTarget("#"+nodeArr[index]));
		});
	},
	...
	deleteConnection: function(connId) {
		...
		layer.confirm(CONFIG.msg.deleteConn, { icon: 7, title: '提示' }, function(index) {
			...
			var e = _base.graph.edge(sourceId, targetId);
	...
	copyNode: function(tempId) {
		...
		// 2、获取选中的节点id数组
		var selectedNodeIdArr = graphUtil.getSelectedNodeIds();
		
		// 3、将选中的节点push到剪贴板
		for (var i = 0, len = selectedNodeIdArr.length; i < len; i++) {
			// 开始节点无法被复制
	...
	pasteNode: function() {
		...
		// 2、获取鼠标位置
		var mousePos = ZFSN.getMousePos(event);
		var top = mousePos.y;
		var left = mousePos.x;
		var tempTop = top;
		var tempLeft = left;
		var copyNodeIdArr = []; // 粘贴生成的新节点的id数组
		
		// 3、粘贴节点
		$.each(_base.myclipboard, function(index) {
			var newNodeId = FLOW.createNewNode(_base.graph.node(_base.myclipboard[index]).nodeType, { 'top': top, 'left': left });
	...
	smallMove: function(moveType) {
		...
		// 获取被选中的节点id列表
		var selectedArr = graphUtil.getSelectedNodeIds();
		// 移动每一个被选中的元素
		for (var i = 0, len = selectedArr.length; i < len; i++) {
			var newTop = $(ZFSN.getJQSel(selectedArr[i])).offset().top + t;
			var newLeft = $(ZFSN.getJQSel(selectedArr[i])).offset().left + l;
	...
	deleteLane: function(tempId) {
		var _base = FLOW._base;
		var id = $(ZFSN.getJQSel(tempId)).parent().attr('id');
