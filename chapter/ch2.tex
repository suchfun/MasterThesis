...
本章对后续章节所用到的相关理论和技术进行简介，主要包括常用的聚类算法（k-means算法和k-medoids算法）以及聚类算法的评估标准、稀疏矩阵的存储方式和多项式拟合技术，对最优化理论进行了简单的阐述，最后简单介绍了马尔科夫链模型。这些理论为后续研究提供了理论基础。
...
一种是针对有聚类标签的数据，将聚类结果与标签进行比较得到的评价指标，称为“外部指标”；另一种不依赖标签数据，针对聚类计算后簇内与簇间的距离度量得到的评价指标，称之为“内部指标”。
...
正则化符合奥卡姆剃刀原理：在所有能够解释观测数据的模型中，模型复杂度最低的模型是最好的模型。

\subsection{梯度下降法}
梯度下降法是求解无约束优化问题的一种方法，若函数可微，从函数的任意一点，通过梯度下降法可以找到函数的局部极小值。

梯度下降的基本思路为：假设实数函数$f(x)$，现在$f(x)$定义域内找一坐标点a，且函数$f(x)$在点a可微，那么函数$f(x)$的值沿着函数在a点的负梯度方向上下降速度最快，即$-\nabla f(\mathbf{a})$所指的方向，则有：
\[\mathbf{b}=\mathbf{a}-\gamma \nabla f\left( \mathbf{a} \right) \]

当$\gamma$其值足够小时，那么一定有$f(\mathbf{a})\geq f(\mathbf{b})$。

基于这一点，我们可以从函数f定义域的任意一点$\mathbf{x}_0$出发，并考虑如下序列$\mathbf{x}_{0}, \mathbf{x}_{1}, \mathbf{x}_{2}, \dots$使得
\[
...
如果采取合适的策略，序列${\textbf{x}_n}$将会收敛函数$f(x)$的局部极小值。在迭代的过程中，$\gamma$的取值是可以改变的，一般会从一个较大的值慢慢减小。

图\ref{GD}示例了这一过程，这里假设$f(x)$定义域为一个二维空间，若假设函数图像类型于一个碗的形状，则函数图像的等高线如图中蓝色线所示，图中红色箭头指向的方向则是在指定坐标点的负梯度方向，沿着红色箭头的方向，将会逐步到达函数图像的底部，也就是函数取局部极小值的点。
\begin{figure}[H]
...
马尔可夫链，又称离散时间马可夫链\citing{norris1998markov}，是一种离散状态空间的随机过程，其状态空间中的状态数是有限的，处于马尔科夫链中的状态会依据转移矩阵中对应的概率分布转移至其他状态或停留在同一状态，但每一次转移过程至于当下所处的状态有关，与当下状态之前所处的所有状态都无关，这种特性称之为“马尔可夫性”，从中可以看出“马尔可夫性”是“无记忆”的。马尔科夫链是最常用的随机过程之一，依据“马尔可夫性”可以对现实生活中的很多模型进行建模，有着很广泛的应用场景。
